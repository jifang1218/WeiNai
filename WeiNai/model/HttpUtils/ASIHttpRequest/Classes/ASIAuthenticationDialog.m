...
+ (void)presentAuthenticationDialogForRequest:(EMASIHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		if ([theRequest authenticationNeeded] == EMASIProxyAuthenticationNeeded) {
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (EMASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (EMASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		EMASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
