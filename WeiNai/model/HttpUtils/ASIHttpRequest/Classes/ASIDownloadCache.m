...
static EMASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";
static NSArray *fileExtensionsToHandleAsHTML = nil;

@interface EMASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation EMASIDownloadCache

+ (void)initialize
{
	if (self == [EMASIDownloadCache class]) {
...
- (void)updateExpiryForRequest:(EMASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	[cachedHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-EMASIHTTPRequest-Expires"];
...
{
  return [EMASIHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}

- (void)storeResponseForRequest:(EMASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	[[self accessLock] lock];
	...
	// Create a special 'X-EMASIHTTPRequest-Expires' header
	...
	NSDate *expires = [self expiryDateForRequest:request maxAge:maxAge];
	if (expires) {
		[responseHeaders setObject:[NSNumber numberWithDouble:[expires timeIntervalSince1970]] forKey:@"X-EMASIHTTPRequest-Expires"];
	}
	...
	[responseHeaders setObject:[NSNumber numberWithInt:statusCode] forKey:@"X-EMASIHTTPRequest-Response-Status-Code"];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(EMASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (NSString *)pathToStoreCachedResponseHeadersForRequest:(EMASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (void)removeCachedDataForRequest:(EMASIHTTPRequest *)request
{
	[self removeCachedDataForURL:[request url]];
}

- (BOOL)isCachedDataCurrentForRequest:(EMASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	if ([self shouldRespectCacheControlHeaders]) {
		...
		// Look for X-EMASIHTTPRequest-Expires header to see if the content is out of date
		NSNumber *expires = [cachedHeaders objectForKey:@"X-EMASIHTTPRequest-Expires"];
		if (expires) {
...
+ (BOOL)serverAllowsResponseCachingForRequest:(EMASIHTTPRequest *)request
{
...
+ (NSString *)keyForURL:(NSURL *)url
{
	...
	// Strip trailing slashes so http://allseeing-i.com/EMASIHTTPRequest/ is cached the same as http://allseeing-i.com/EMASIHTTPRequest
...
- (BOOL)canUseCachedDataForRequest:(EMASIHTTPRequest *)request
{
