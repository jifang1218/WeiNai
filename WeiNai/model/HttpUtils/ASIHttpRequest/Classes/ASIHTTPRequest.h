...
@class EMASIDataDecompressor;

extern NSString *EMASIHTTPRequestVersion;
...
typedef enum _ASIAuthenticationState {
	EMASINoAuthenticationNeededYet = 0,
	EMASIHTTPAuthenticationNeeded = 1,
	EMASIProxyAuthenticationNeeded = 2
} EMASIAuthenticationState;
...
// The error domain that all errors generated by EMASIHTTPRequest use
extern NSString* const EMNetworkRequestErrorDomain;
...
extern unsigned long const EMASIWWANBandwidthThrottleAmount;
...
@interface EMASIHTTPRequest : NSOperation <NSCopying> {
	...
	// Generally, you won't use this directly, but EMASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	...
	// Set to true when EMASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = EMASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	// If you are using Basic authentication and want to force EMASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, EMASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, EMASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - EMASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	// Also see the comments in EMASINetworkQueue.h
	...
	// Tells EMASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let EMASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// EMASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See EMASIAuthenticationState values above. 0 == default == No authentication needed yet
	EMASIAuthenticationState authenticationNeeded;
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops EMASIHTTPRequest cleaning up a temporary post body)
	...
	// The download cache that will be used for this request (use [EMASIHTTPRequest setDefaultCache:cache] to configure a default cache
	...
	//
	// Setting this to NO may be especially useful for users using EMASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
...
// On iPhone, EMASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign, readonly) EMASIAuthenticationState authenticationNeeded;
...
#endif
@property (retain) EMASIDataDecompressor *dataDecompressor;
