...
+ (id)inputStreamWithFileAtPath:(NSString *)path request:(EMASIHTTPRequest *)theRequest
{
...
+ (id)inputStreamWithData:(NSData *)data request:(EMASIHTTPRequest *)theRequest
{
	EMASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask EMASIHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([EMASIHTTPRequest isBandwidthThrottled]) {
		toRead = [EMASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[EMASIHTTPRequest incrementBandwidthUsedInLastSecond:rv];
	return rv;
}
