...
// An xPath query that controls the external resources EMASIWebPageRequest will fetch
...
- (void)finishedFetchingExternalResources:(EMASINetworkQueue *)queue;
- (void)externalResourceFetchSucceeded:(EMASIHTTPRequest *)externalResourceRequest;
- (void)externalResourceFetchFailed:(EMASIHTTPRequest *)externalResourceRequest;

@property (retain, nonatomic) EMASINetworkQueue *externalResourceQueue;
@property (retain, nonatomic) NSMutableDictionary *resourceList;
@end

@implementation EMASIWebPageRequest

+ (void)initialize
{
	if (self == [EMASIWebPageRequest class]) {
...
- (void)parseAsCSS
{
	...
	if (err) {
		[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:100 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to read HTML string from response",NSLocalizedDescriptionKey,err,NSUnderlyingErrorKey,nil]]];
		return;
	} else if (!responseCSS) {
		[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:100 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to read HTML string from response",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	[self setExternalResourceQueue:[EMASINetworkQueue queue]];
	...
	for (NSString *theURL in [[self resourceList] keyEnumerator]) {
		EMASIWebPageRequest *externalResourceRequest = [EMASIWebPageRequest requestWithURL:[NSURL URLWithString:theURL relativeToURL:[self url]]];
...
- (void)parseAsHTML
{
    ...
    if (doc == NULL) {
		[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:101 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Error: unable to parse reponse XML",NSLocalizedDescriptionKey,nil]]];
		return;
    }
...
- (void)finishedFetchingExternalResources:(EMASINetworkQueue *)queue
{
	if ([self urlReplacementMode] != ASIDontModifyURLs) {
		if (webContentType == ASICSSWebContentType) {
			...
			if (err) {
				[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:101 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Error: unable to read response CSS from disk",NSLocalizedDescriptionKey,nil]]];
				return;
			}
			if (![self error]) {
			...
			if ([self downloadDestinationPath]) {
				...
				if (err) {
					[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:101 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Error: unable to write response CSS to disk",NSLocalizedDescriptionKey,nil]]];
					return;
				}
			} else {
...
- (void)readResourceURLs
{
    ...
    if(xpathCtx == NULL) {
		[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:101 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Error: unable to create new XPath context",NSLocalizedDescriptionKey,nil]]];
		return;
    }
    ...
    if(xpathObj == NULL) {
		...
		[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:101 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Error: unable to evaluate XPath expression!",NSLocalizedDescriptionKey,nil]]];
		return;
    }
...
- (NSString *)contentForExternalURL:(NSString *)theURL
{
	...
	if (data && contentType) {
		...
		dataURI = [dataURI stringByAppendingString:[EMASIHTTPRequest base64forData:data]];
		return dataURI;
	}
	return nil;
}

- (NSString *)cachePathForRequest:(EMASIWebPageRequest *)theRequest
{
	// If we're using a download cache (and its a good idea to do so when using EMASIWebPageRequest), ask it for the location to store this file
