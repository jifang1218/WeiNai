...
@implementation EMASIS3ObjectRequest

- (EMASIHTTPRequest *)HEADRequest
{
	EMASIS3ObjectRequest *headRequest = (EMASIS3ObjectRequest *)[super HEADRequest];
...
{
	EMASIS3ObjectRequest *newRequest = [[[self alloc] initWithURL:nil] autorelease];
...
{
	EMASIS3ObjectRequest *newRequest = [self requestWithBucket:theBucket key:theKey];
...
+ (id)PUTRequestForFile:(NSString *)filePath withBucket:(NSString *)theBucket key:(NSString *)theKey
{
	...
	[newRequest setRequestMethod:@"PUT"];
	[newRequest setMimeType:[EMASIHTTPRequest mimeTypeForFileAtPath:filePath]];
	return newRequest;
}
...
{
	EMASIS3ObjectRequest *newRequest = [super copyWithZone:zone];
...
{
	if ([self subResource]) {
		[self setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@://%@.%@%@?%@",[self requestScheme],[self bucket],[[self class] S3Host],[EMASIS3Request stringByURLEncodingForS3Path:[self key]],[self subResource]]]];
	} else {
		[self setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@://%@.%@%@",[self requestScheme],[self bucket],[[self class] S3Host],[EMASIS3Request stringByURLEncodingForS3Path:[self key]]]]];
	}
}
...
- (NSString *)mimeType
{
	if (mimeType) {
		...
		return [EMASIHTTPRequest mimeTypeForFileAtPath:[self postBodyFilePath]];
	} else {
...
- (NSString *)canonicalizedResource
{
	if ([[self subResource] length] > 0) {
		return [NSString stringWithFormat:@"/%@%@?%@",[self bucket],[EMASIS3Request stringByURLEncodingForS3Path:[self key]], [self subResource]];
	}
	return [NSString stringWithFormat:@"/%@%@",[self bucket],[EMASIS3Request stringByURLEncodingForS3Path:[self key]]];
}
...
- (NSMutableDictionary *)S3Headers
{
	...
	if ([self sourceKey]) {
		NSString *path = [EMASIS3Request stringByURLEncodingForS3Path:[self sourceKey]];
