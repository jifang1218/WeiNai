...
// Private stuff
@interface EMASIS3Request ()
	+ (NSData *)HMACSHA1withKey:(NSString *)key forString:(NSString *)string;
@end

@implementation EMASIS3Request
...
{
	[self setDateString:[[EMASIS3Request S3RequestDateFormatter] stringFromDate:date]];	
}

- (EMASIHTTPRequest *)HEADRequest
{
	EMASIS3Request *headRequest = (EMASIS3Request *)[super HEADRequest];
...
- (void)buildRequestHeaders
{
	...
	if (![self accessKey]) {
		[self setAccessKey:[EMASIS3Request sharedAccessKey]];
	}
	if (![self secretAccessKey]) {
		[self setSecretAccessKey:[EMASIS3Request sharedSecretAccessKey]];
	}
	...
	NSString *signature = [EMASIHTTPRequest base64forData:[EMASIS3Request HMACSHA1withKey:[self secretAccessKey] forString:stringToSign]];
...
{
	[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:ASIS3ResponseParsingFailedType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Parsing the resposnse failed",NSLocalizedDescriptionKey,parseError,NSUnderlyingErrorKey,nil]]];
}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
	...
	if ([elementName isEqualToString:@"Message"]) {
		[self failWithError:[NSError errorWithDomain:EMNetworkRequestErrorDomain code:ASIS3ResponseErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[self currentXMLElementContent],NSLocalizedDescriptionKey,nil]]];
...
{
	EMASIS3Request *newRequest = [super copyWithZone:zone];
